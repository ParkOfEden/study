
-- 기존 테이블 삭제 (존재할 경우만)
DROP TABLE ACCOUNTS PURGE;

CREATE TABLE ACCOUNTS (
num NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
id VARCHAR2(50) NOT NULL UNIQUE, -- 로그인 아이디
pass VARCHAR2(100) NOT NULL, -- 비밀번호
nickname VARCHAR2(50), -- 닉네임 (중복 허용)
name VARCHAR2(50) NOT NULL, -- 이름
addr VARCHAR2(200), -- 주소
phone VARCHAR2(20), -- 전화번호
gender VARCHAR2(10), -- 성별
age NUMBER, -- 나이
email VARCHAR2(100), -- 이메일
profile_blob BLOB, -- 프로필 이미지 바이너리 데이터
kakao_id VARCHAR2(100), -- 카카오 고유 ID
login_type VARCHAR2(20) DEFAULT 'NORMAL', -- 로그인 구분
created_at TIMESTAMP DEFAULT SYSTIMESTAMP
);

-- 프로필 이미지(BLOB), 카카오 ID, 로그인 타입 컬럼 추가
ALTER TABLE ACCOUNTS ADD (
    profile_blob BLOB,
    kakao_id     VARCHAR2(100),
    login_type   VARCHAR2(20) DEFAULT 'NORMAL'
);

-- 확인용 (구조 변경 확인)
DESC ACCOUNTS;
SELECT num, id, nickname, name, email, login_type, created_at 
FROM ACCOUNTS 
ORDER BY num DESC; -- 최근 가입한 순서대로 보기

-- 특정 이메일 '1@1.com'을 가진 계정의 모든 정보 조회
SELECT * FROM ACCOUNTS
WHERE email = '1@1.com';

-- 특정 이메일 '1@1.com'을 가진 계정 삭제
DELETE FROM ACCOUNTS 
WHERE email = '1@1.com';

-- 변경 사항 반영
COMMIT;

-- 1. 관리자(admin) 계정에 닉네임 및 로그인 타입 부여
UPDATE ACCOUNTS 
SET nickname = '관리자', 
    login_type = 'ADMIN' 
WHERE id = 'admin';

-- 2. 임의의 유저 1,000명 생성 (기존에 데이터가 없다면 실행)
INSERT INTO ACCOUNTS (id, pass, nickname, name, addr, phone, gender, age, email, login_type)
SELECT 
    'user' || LEVEL,
    'pass' || LEVEL,
    'user' || LEVEL,
    '사용자' || LEVEL,
    '서울시',
    '010-0000-0000',
    CASE WHEN MOD(LEVEL, 2) = 0 THEN '남성' ELSE '여성' END,
    20 + MOD(LEVEL, 30),
    'user' || LEVEL || '@naver.com',
    'NORMAL'
FROM DUAL
CONNECT BY LEVEL <= 1000;

COMMIT;
---------------------------------------------------------------------------
--	여기서부터 추가된 내용 													 --
---------------------------------------------------------------------------
-- 기존 테이블을 삭제하지 않고 닉네임 column만 추가
ALTER TABLE ACCOUNTS
ADD nickname VARCHAR2(50);

-- 확인용
SELECT * FROM ACCOUNTS;

-- admin에게 '관리자' 닉네임 부여
UPDATE ACCOUNTS
SET nickname = '관리자'
WHERE id = 'admin';

-- 변경사항 반영
COMMIT;

-- 임의로 1,000명 생성 (닉네임 포함)
INSERT INTO ACCOUNTS (
    id, pass, name, addr, phone, gender, age, email, nickname
)
SELECT 
    'user' || LEVEL,                     -- id: user1, user2...
    'pass' || LEVEL,                     -- pass: pass1, pass2...
    '사용자' || LEVEL,                    -- name: 사용자1, 사용자2...
    '서울시 ' || MOD(LEVEL, 25) || '구',    -- addr
    '010-1234-' || LPAD(LEVEL, 4, '0'),  -- phone: 010-1234-0001...
    CASE WHEN MOD(LEVEL, 2) = 0 THEN '남성' ELSE '여성' END, -- 성별
    20 + MOD(LEVEL, 30),                 -- 나이 20~50 사이
    'user' || LEVEL || '@naver.com',      -- email
    'user' || LEVEL                     -- nickname: id와 동일하게 설정
FROM DUAL
CONNECT BY LEVEL <= 1000;

COMMIT;

-- 기존 테이블 삭제 (존재할 경우만)
DROP TABLE ACCOUNTS PURGE;


-- 테이블 생성
CREATE TABLE ACCOUNTS (
    num        NUMBER GENERATED BY DEFAULT ON NULL AS IDENTITY PRIMARY KEY,
    id         VARCHAR2(50) NOT NULL UNIQUE,   -- 로그인 아이디
    nickname   VARCHAR2(50),                   -- 닉네임 (선택 사항)
    pass       VARCHAR2(100) NOT NULL,         -- 비밀번호
    name       VARCHAR2(50) NOT NULL,          -- 이름
    addr       VARCHAR2(200),                  -- 주소
    phone      VARCHAR2(20),                   -- 전화번호
    gender     VARCHAR2(10),                   -- 성별
    age        NUMBER,                         -- 나이
    email      VARCHAR2(100),                  -- 이메일
    created_at TIMESTAMP DEFAULT SYSTIMESTAMP
);

-- 닉네임 칸 추가
ALTER TABLE users ADD COLUMN nickname VARCHAR(50) DEFAULT NULL;

-- 관리자 계정 추가 (닉네임 포함)
INSERT INTO ACCOUNTS (
    id, nickname, pass, name, addr, phone, gender, age, email
) VALUES (
    'admin', 'admin', 'admin', 'MASTER',
    '부산광역시', '01011111111', '남성', 30, 'ADMIN@admin.com'
);

-- 변경사항 반영
COMMIT;

-- 확인용 일반 계정 추가 (닉네임을 ID와 동일하게 설정)
INSERT INTO ACCOUNTS (
    id, nickname, pass, name, addr, phone, gender, age, email
) VALUES (
    '1', '1', '1', '일반계정',
    '부산광역시', '01011121111', '남성', 30, 'normal@normal.com'
);  '2','2','2' '일반계정',
    '부산광역시' '010-86616807', '남성', 26, 'cheolwon18@gmail.com'

COMMIT;

-- 확인용
SELECT * FROM ACCOUNTS;
-- 임의로 1,000명 생성 (닉네임 포함)
INSERT INTO ACCOUNTS (
    id, nickname, pass, name, addr, phone, gender, age, email
)
SELECT 
    'user' || LEVEL,                     -- id: user1, user2...
    'user' || LEVEL,                     -- nickname: id와 동일하게 설정
    'pass' || LEVEL,                     -- pass: pass1, pass2...
    '사용자' || LEVEL,                    -- name: 사용자1, 사용자2...
    '서울시 ' || MOD(LEVEL, 25) || '구',    -- addr
    '010-1234-' || LPAD(LEVEL, 4, '0'),  -- phone: 010-1234-0001...
    CASE WHEN MOD(LEVEL, 2) = 0 THEN '남성' ELSE '여성' END, -- 성별
    20 + MOD(LEVEL, 30),                 -- 나이 20~50 사이
    'user' || LEVEL || '@naver.com'      -- email
FROM DUAL
CONNECT BY LEVEL <= 1000;

COMMIT;

DELETE FROM ACCOUNTS 
WHERE email = 'cheolwon18@gmail';

COMMIT

-- 확인용
SELECT COUNT(*) FROM ACCOUNTS;
SELECT * FROM ACCOUNTS WHERE email = 'cheolwon18@gmail.com';

---------------------------------------------------------------------------
--	닉네임 관련 sql   													 --
---------------------------------------------------------------------------
-- 닉네임 중복 확인 sql
ALTER TABLE ACCOUNTS
ADD CONSTRAINT ACCOUNTS_NICKNAME_UNIQUE UNIQUE (nickname);

-- 일반/카카오 로그인 구분 sql
ALTER TABLE ACCOUNTS ADD kakao_id VARCHAR2(100);
ALTER TABLE ACCOUNTS ADD login_type VARCHAR2(20);