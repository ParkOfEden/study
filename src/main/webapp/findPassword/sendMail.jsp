...
<% 
	...
	//STARTTLS 명령을 사용하여 TLS 암호화 연결 사용여부 지정
	...
	MimeMessage message =new MimeMessage(mailSession);
		...
		//CC: 참조
		//BCC: 숨은 참조
		message.setRecipient(Message.RecipientType.TO, new InternetAddress("studylsy@gmail.com"));
		//발신자 설정
		message.setFrom(new InternetAddress("Chlrlms@gmail.com"));
		//제목 설정(제목, 인코딩)
		message.setSubject("메일 테스트","utf-8");
		//본문 텍스트 설정
		message.setText("안녕하세요?", "UTF-8");
		
		//메일 발신 요청
		Transport.send(message);// 메일 발신 실패시 예외 발생
		//Mail Server에서 메일 발신을 처리할때 까지 blocking(대기)
